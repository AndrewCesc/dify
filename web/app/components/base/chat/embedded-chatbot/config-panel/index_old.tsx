            ...
            !showConfigPanelBeforeChat && collapsed && (
              <>
                ...
                <div className='grow py-[3px] text-[13px] text-indigo-600 leading-[18px] font-medium'>
                  {t('share.chat.configStatusDes', { lng: language })}
                </div>
                <Button
                ...
                >
                  ...
                  {t('common.operation.edit', { lng: language })}
                </Button>
              </>
            )
          }
          {
            !showConfigPanelBeforeChat && !collapsed && (
              <>
                <Star06 className='mr-1 mt-1 w-4 h-4 text-indigo-600' />
                <div className='grow py-[3px] text-[13px] text-indigo-600 leading-[18px] font-medium'>
                  {t('share.chat.privatePromptConfigTitle', { lng: language })}
                </div>
              </>
            )
          }
          ...
          !collapsed && !showConfigPanelBeforeChat && (
            <div className='p-6 rounded-b-xl'>
              ...
              <div className={cn('pl-[136px] flex items-center', isMobile && '!pl-0')}>
                ...
                >
                  {t('common.operation.save', { lng: language })}
                </Button>
                <Button
                  onClick={() => setCollapsed(true)}
                >
                  {t('common.operation.cancel', { lng: language })}
                </Button>
              </div>
            </div>
          )
          ...
          showConfigPanelBeforeChat && (
            <div className='p-6 rounded-b-xl'>
              ...
              >
                ...
                {t('share.chat.startChat', { lng: language })}
              </Button>
            </div>
          )
        }
      </div>
      {
        showConfigPanelBeforeChat && (site || customConfig) && (
          <div className='mt-4 flex flex-wrap justify-between items-center py-2 text-xs text-gray-400'>
            {site?.privacy_policy
              ? <div className={cn(isMobile && 'mb-2 w-full text-center')}>{t('share.chat.privacyPolicyLeft', { lng: language })}
                <a
                  className='text-gray-500 px-1'
                  href={site?.privacy_policy}
                  target='_blank' rel='noopener noreferrer'>{t('share.chat.privacyPolicyMiddle', { lng: language })}</a>
                {t('share.chat.privacyPolicyRight', { lng: language })}
              </div>
              : <div>
              </div>}
            {
